...
else
{
    ...
    $_REQUEST['category']   = !empty($_REQUEST['category'])   ? trim($_REQUEST['category'])   : '';
    ...
    $categories = !empty($category)               ? ' AND ' . get_childrens($category)    : '';
	...
	/**场合**/
	//var_dump($_REQUEST['oc']);
	if (!empty($_REQUEST['oc']))
    {
       $attr_id = 3;
	   $arr_oc = array('1'=>'生日', '2'=>'聚会', '3'=>'感恩', '4'=>'庆祝', '5'=>'活动', '6'=>'商务', '7'=>'狂欢');
	   $sel_oc = explode('.',$_REQUEST['oc']);
	   $ai =1;
	   $acon ='';
	   $flag_oc = array();
	   $sql = "SELECT distinct goods_id FROM " . $ecs->table("goods_attr") . " as ga WHERE 1  and ga.attr_id='$attr_id'  and exists ( ";
	   foreach($sel_oc as $v)
	   {
		   $v = intval($v);
		   $flag_oc[$v] =1;
		   ...
		   if($ai ==1)
		   {
			  $sql .= " select 1 from " .$ecs->table("goods_attr") . "as e1 ";
			   $acon .=" and e1.goods_id=ga.goods_id and (e1.attr_id='$attr_id' and e1.attr_value='$attr_val') ";
		   }
		   else
		   {
			  $sql .= " left join " .$ecs->table("goods_attr") . "as e{$ai} on  e{$ai}.goods_id= e{$ai_pre}.goods_id  ";
			  $acon .=" and (e{$ai}.attr_id='$attr_id' and e{$ai}.attr_value='$attr_val') ";
		   }
		   $ai++;
		}
		$sql .= " where 1 $acon ";						
	...
	if (!empty($_REQUEST['cat']))
    {
	   ...
	   $categories .= !empty($arr_cat) ? ' AND ' . get_childrens($arr_cat)    : '';
	   $smarty->assign('flag_cat', $flag_cat);
	}
    ...
    {
       $attr_id = 8;
	   $arr_cf = array('1'=>'坚果','2'=>'含酒');
	   $sel_cf = explode('.',$_REQUEST['cf']);
	   $ai =1;
	   $acon ='';
	   $flag_cf = array();
	   $sql = "SELECT distinct goods_id FROM " . $ecs->table("goods_attr") . " as ga WHERE 1 and ga.attr_id='$attr_id' and exists ( ";
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name, g.goods_desc, g.market_price,left(g.goods_sn,3) as goods_sn, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            "WHERE g.is_delete = 0 AND g.goods_id in(1,5,6,7,8,9,11,27,22,26,25,19,17,16,15,20,18,21,30,32,35,37,38,33) AND g.is_alone_sale = 1 $attr_in ".
                "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .
            "ORDER BY $sort $order";
    //var_dump($sql);
	...
	if(isset($_REQUEST['is_ajax']) && $_REQUEST['is_ajax'] ==1)
	{
		...
		$result['content'] = $smarty->fetch('library/search_goods_list.lbi');
		ob_clean();
