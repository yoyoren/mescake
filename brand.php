...
if (!$smarty->is_cached('brand.dwt', $cache_id))
{
	...
	if($brand_info['brand_id']==4)
		{	
			$brand_info['brand_logo']='images/all.png';
		}
	//print_r($brand_info);
    if (empty($brand_info))
    {
...
{
	//all页面
	$all_goods=$db->getAll(' select * from ecs_goods where goods_id in(1,5,6,7,8,9,11,27,22,26,25,19,17,16,15,20,18,21,30,32,35,37,38,33) and is_on_sale=1');
...
function brand_recommend_goods($type, $brand, $cat = 0)
{
    ...
    if ($result === NULL)
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_desc,g.market_price, g.shop_price AS org_price, g.promote_price, ' .
...
function goods_count_by_brand($brand_id, $cate = 0)
{
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('goods'). ' AS g '.
            "WHERE brand_id = '$brand_id'  AND g.is_alone_sale = 1 AND g.is_delete = 0";
...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.is_on_sale,g.goods_id,left(g.goods_sn,3) as goods_sn,g.goods_desc, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief,g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            "WHERE g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_sugar=1 AND g.brand_id = '$brand_id' $cate_where".
            "ORDER BY $sort $order";
