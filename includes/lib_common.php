...
function get_childrens($catids)
{
	...
	foreach($catids as $cat)
    {
		...
		{
		 $all_cat[]=$cat;
		 $all_cat = array_merge($all_cat, array_keys(cat_list($cat, 0, false)));
		}
    }
	$all_cat = array_unique($all_cat);
	return 'g.cat_id ' . db_create_in($all_cat);
}
 ...
 * @return  string
 */
function price_format_new($price, $change_price = true)
{
    $price = number_format($price, 2, '.', '');
	$price = explode('.', (string)$price );
    return '<strong>' . $price[0] . '</strong>.' . $price[1].'RMB';
}
...
function get_virtual_goods($order_id, $shipping = false)
{
    if ($shipping)
    {
        $sql = 'SELECT goods_id, goods_name,  extension_code FROM '.
           $GLOBALS['ecs']->table('order_goods') .
           " WHERE order_id = '$order_id' AND extension_code > ''";
    }
    else
    {
        $sql = 'SELECT goods_id, goods_name, (goods_number) AS num, extension_code FROM '.
           $GLOBALS['ecs']->table('order_goods') .
           " WHERE order_id = '$order_id' AND is_real = 0 AND (goods_number) > 0 AND extension_code > '' ";
    }
    $res = $GLOBALS['db']->getAll($sql);
...
function log_mcard_change($user_id, $user_money = 0, $change_desc = '',$pay_points = 0,$order_id=0,$change_type = 1)
{
	...
	log_account_change($user_id, $user_money, 0, 0, $pay_points, $change_desc);
}
?>