...
function order_fee($order, $goods, $consignee)
{
    ...
    foreach ($goods AS $val)
    {
        ...
        if($val['goods_id'] == 61 || $val['goods_id'] == 60)
		{
		    $total['pack_fee']=$pack_fee += $val['goods_price'] * $val['goods_number'];		      
		}
		else
		{
    ...
    $total['goods_price_formated']  = price_format($total['goods_price']+$pack_fee, false);
...
//    $total['pack_fee_formated'] = price_format($total['pack_fee'], false);
	...
	$address=$_SESSION['flow_consignee']['address'];
...
function get_goods_attr_info($arr, $type = 'pice')
{
    ...
    if (!empty($arr))
    {
		...
		$sql = "SELECT  ga.attr_value, ga.attr_price ".
 ...
 * @return  bool
 */
function use_bonus($bonus_id, $order_id,$user_id,$reusable=0)
{
    if($reusable==0)
 	{
