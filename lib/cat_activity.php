...
class MES_Cat_activity{
	...
	public static function get_all(){
		GLOBAL $db;
		$sql= "select * from cat_activity  order by add_time desc";
		$data = $db->getAll($sql);
	...
	public static function cat_get_by_status($status=0){
		GLOBAL $db;
		$sql= "select * from cat_activity where status={$status} order by add_time desc";
		$data = $db->getAll($sql);
		for($i=0;$i<count($data);$i++){
			 $data[$i]['format_date'] = date('Y-m-d H:i:s', $data[$i]['add_time']);
		}
	...
	public static function cat_get_by_id($id=0){
		GLOBAL $db;
		$sql= "select * from cat_activity where id={$id}";
		$data = $db->getAll($sql);
		return $data[0];
	}
