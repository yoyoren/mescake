...
//    }
   ...
   function get_code($order, $payment,$auto=false)
   {
       ...
       $page_url           = '';
       $bg_url             = return_url(basename(__FILE__,'.php')).'&';
       $version            = 'v2.0';
       $language           = 1;
       ...
       $pid                = '';
        ...
        //$def_url .= "<input type='submit' name='submit1' value='" . $GLOBALS['_LANG']['pay_button'] . "' />";
        $def_url .= "</form></div></br>";
    ...
    function respond()
    {
        ...
        if (strtoupper($sign_msg) == strtoupper($merchant_signmsg))
        {
            ...
            {
                order_paid($ext1);
				echo '<html><body><div style="display:none"><result>1</result><redirecturl>http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?code=kuaiqian </redirecturl></div></body></html>';
                return true;
            }
            else
            {
