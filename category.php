...
function get_userinfo($id){
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.user_name,u.mobile_phone,ua.province,ua.city,ua.district,ua.address'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u, '
             .$GLOBALS['ecs']->table('user_address').' AS ua'.
            " WHERE u.address_id=ua.address_id and u.user_id = '$id'";
    $user = $GLOBALS['db']->getRow($sql);
    $sql1="select region_name as province from ".$GLOBALS['ecs']->table('region')." where region_id=".$user['province'];
    $province=$GLOBALS['db']->getOne($sql1);
    $sql2="select region_name as province from ".$GLOBALS['ecs']->table('region')." where region_id=".$user['city'];
    $city=$GLOBALS['db']->getOne($sql2);
    $sql3="select region_name as province from ".$GLOBALS['ecs']->table('region')." where region_id=".$user['district'];
    $district=$GLOBALS['db']->getOne($sql3);
    $user['user_address']=$province.$city.$district.$user['address'];
    return $user;
}
