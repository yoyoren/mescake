...
class ucenter extends integrate
{
     ...
     * @return void
     */
    function login11($username, $password)
    {
        ...
        {
            //检查用户是否存在,不存在直接放入用户表
            $result = $this->db->getRow("SELECT user_id,ec_salt FROM " . $GLOBALS['ecs']->table("users") . " WHERE user_name='$username' or mobile_phone='$username'");
