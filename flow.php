...
//print_r($card_message);
	...
	*/
    ...
    /* 保存 session */
    $_SESSION['flow_order'] = $order;//echo "<pre>";print_r($order);
}
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	if($_POST['pay_id']==1) $pay_id=4;
	if($_POST['pay_id']==2) $pay_id=4;
	if($_POST['pay_id']==3) $pay_id=2;
	if($_POST['pay_id']==4) $pay_id=3;
   ...
   /* 判断是不是实体商品 
    ...
    if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0)
    {
        ...
        else
        {
			...
			$order['pay_id']=4;$order['pay_note']="现金:0";
...
//    while ($error_no == 1062); //如果是订单号重复则重新提交数据
	...
	foreach($cart_goods as $val)
	{
		if($val['goods_price'] > 100)
		{
		   ...
		   if ($attr == '15.') $num = 50;
		   if ($attr == '20.') $num = 80;
		   if ($attr == '25.') $num = 100;			           if ($attr == '30.') $num = 120;
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                ...
                "goods_price, goods_attr, goods_discount,is_integral) ".
            " SELECT '$new_order_id', goods_id, goods_name, goods_sn, goods_number,  ".
                "goods_price, goods_attr, if(is_integral,-1,1) ,is_integral".
            " FROM " .$ecs->table('cart') .
	...
	if($sends>0)
	{
       $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                "order_id, goods_id, goods_name, goods_sn, goods_number,  ".
                "goods_price, goods_attr, goods_discount,is_integral) ".
            " values('$new_order_id', 60, '餐具套装', '00', '$sends','0', '',1,0) ";
       $db->query($sql);
	}
		if(isset($_COOKIE['emar']))
	{
        ...
        $cps -> setOrderTime($add_time);  // 设置下单时间
        $cps -> setUpdateTime($add_time); // 设置订单更新时间，如果没有下单时间，要提前对接人提前说明
        $cps -> setCampaignId($arr[2]);                 // 测试时使用"101"，正式上线之后活动id必须要从cookie中获取
        $cps -> setFeedback($arr[3]);			// 测试时使用"101"，正式上线之后活动id必须要从cookie中获取
        $cps -> setFare($order['shipping_fee']);
        $cps -> setFavorable($order['bonus']+$order['surplus']);                   // 设置优惠券
		$cps -> setOrderStatus($order['order_status']);             // 设置订单状态
        $cps -> setPaymentStatus($order['pay_status']);   				// 设置支付状态
        $cps -> setPaymentType($order['pay_name']);		// 支付方式
		...
		$sql = "INSERT INTO cps ( " .
                "order_id,src,channel,cid,wi,order_time)".
                " values('$new_order_id','emar','cps',".$arr[2].",'".$arr[3]."','".$ordertime."') ";
       $db->query($sql);
...
//print_r($rowss);
$smarty->assign('accepter', $rowss);//送货地址	
