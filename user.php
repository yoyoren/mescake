...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','check_mobile','clear_history','qpassword_name', 'get_passwd_question', 'check_answer','send_captcha','check_captcha','got_verify');
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        include_once(ROOT_PATH . 'includes/lib_passport.php');
         ...
         //$username = isset($_POST['username']) ? trim($_POST['username']) : trim($_POST['email']);
		...
		$other['rea_name'] = isset($_POST['rea_name']) ? $_POST['rea_name'] : '';
		...
		//真实姓名
		/* if (strlen($other['rea_name']) < 6)
        {
            show_message($_LANG['passport_js']['rea_name_shorter']);
        }*/
        /* 验证码检查 */
...
*/
            /* 检查验证码 */
       /* include_once('includes/cls_randNum.php');
        $validator = new randNum();
        ...
        if (register($username, $password, $email,$other) !== false)
        {
			...
			if(time()>=1386086400 && time()<1388505600)
			{
				$res=$db->query("update ecs_users set user_money=50 where user_name='$username'");
				if($res)
				{	
					$charge_num=$db->query("update ecs_users set charge_num=1 where user_name='$username'");	
					$u_id=$db->getOne("select user_id from ecs_users where user_name='$username'");
					$current_time=time();
					$res2=$db->query("insert into mcard_log(user_id,change_type,user_money,change_desc,change_time)values({$u_id},2,50.00,'2013圣诞礼遇赠送50元',{$current_time}) ");
					/*echo "<script language='javascript'>";
					echo "window.alert('已有50元现金赠送到您的账号，截止使用日期为2014年1月31号');";
					echo "window.location='index.php';";
					echo "</script>";*/
					$jump=$_SESSION['jump'];
					$gid=$_SESSION['goods_ids'];
					if($gid =='')
					{
					...
					else
					{
						...
						{	
							echo "<script language='javascript'>";						
							echo "window.location='flow.php?step=cart';";
							echo "</script>";							
						}	
						if($jump==2)
						{
							echo "<script language='javascript'>";						
							echo "window.location='shopcar.php';";
							echo "</script>";
						}
					}
...
elseif ($action == 'signin')
{
    ...
    if ($user->login11($username, $password))
    {
        ...
        //$result['content'] = $smarty->fetch('library/denglu.lbi');
    }
    else
    {
        $_SESSION['login_fail']++;
...
elseif ($action == 'profile')
{
	...
	$user_info['province_list'] = get_regions();
...
elseif ($action == 'act_edit_profile')
{
	...
	$mobile_phone = isset($_POST['mobile_phone']) ? trim($_POST['mobile_phone']) : $mobile_phone;
...
elseif ($action == 'act_edit_password')
{
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    {
        ...
        if ($user->edit_user(array('username'=> (empty($code) ? $_SESSION['user_name'] : $user_info['user_name']), 'old_password'=>$old_password, 'password'=>$new_password), empty($code) ? 0 : 1))
        {
			...
			$db->query($sql);
            //$user->logout();
            show_message($_LANG['edit_password_success'], '我的信息', 'user.php?act=profile', 'info');
        }
        else
        {
...
elseif ($action == 'account_detail')
{
	...
	$surplus_amount =$db->getOne("select user_money from ecs_users where user_id={$user_id}");
...
/* 发送验证*/
else if ($action == 'send_captcha'){
	include_once('includes/cls_json.php');
    $json = new JSON;
	$result=array('err'=>0,'capt'=>'','type'=>$_GET['type']);
	$mobile=!empty($_GET['mobile']) ? $_GET['mobile'] :'';
	$type  =!empty($_GET['type'])   ? $_GET['type']   :'';
	$exit=($user->check_mobile($mobile))?1:0;
	if($type=='1'){
		if($exit) 
		{
			$send=1;
		}else{
			$result['err']="您输入的手机号不存在，请核对后重新输入";
		}
	}else{
		if(!$exit) 
		{
			...
			$result['err']="此手机号已经被注册，请重新输入";
		}
	}
	if($send)
	{
		require_once 'includes/cls_randNum.php';	
		$randNum=new randNum();
...
elseif($action=="check_card"){
	$cardpassword=$_POST['card_password'];
	$cardid=$_POST['card_num'];
    $sql="select mc.* from moneycards" 
	." as mc where "
	."mc.cardid='".$cardid."' and mc.cardpassword='".$cardpassword."'";
	$record = $db->getRow($sql);//print_r($record);
		...
		if($record['flag']==1)
		{
			if($record['user_id']==0 && $record['used_time']==0)
			{
				...
				if($time<$record['sdate'])
				{
					...
					echo "4";//储值卡已过期
				}elseif($verify!=$_SESSION['captcha']){//验证验证码
					echo "6";
				}
			}else{
				echo "3";//储值卡已使用
...
elseif($action == 'send_verify'){
	...
	$randNum->record_word($word);
	if(!empty($mobile) && !empty($word)){
		$randNum->sendsms($mobile,$word,$num);
	}else{
		echo "0";
	}
}
...
elseif($action == 'chargecard_do')
{
	...
	{
	   $result['error'] =1;
	   $result['message'] ='卡号不能为空';
	   die($json->encode($result));
	}
	if(! is_numeric( $card_num ) || strlen( $card_num ) != 16)
	{
	   $result['error'] =2;
	   $result['message'] ='卡号格式错误，请重新输入';
	   die($json->encode($result));
	}
	...
	$old_mobile=$db->getOne($sql);
	if($mobile != $old_mobile)
	{
			$sql="update ecs_users set mobile_phone='{$_POST['mobile']}' where user_id=".$_SESSION['user_id'];
			$db->query($sql);		
	}*/
	//注册时间
