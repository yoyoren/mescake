...
if (obj.error)
  ...
  {
    orderSelectedResponse(obj.content);
	document.getElementById('bonus_id').value = obj.bonus_id;
  }
}
//改变礼金卡
function changecard(is_surplus)
{
  Ajax.call('flow.php?step=validate_gift', 'is_surplus=' + is_surplus, changecardResponse, 'GET', 'JSON');
}

function changecardResponse(obj)
{
...
if (obj.error)
  {
    ...
    try
    {
      document.getElementById('surplus').value = '0';
    }
    catch (ex) { }
  }
  else
  {
    orderSelectedResponse(obj.content);
	document.getElementById('surplus').value = obj.surplus;
	//alert(obj.surplus);
  }
}
...
function checkOrderForm(frm)
{
   if(document.getElementById('save').value=="1")
   {
	   if(!checkConsignee(frm))
	   {
			...
			frm.action = frm.action + '?step=checkout';
			return true;
	   }
   }else{
	   document.getElementById('meg').innerHTML='请保存送货信息';
	   return false;
   }
...
//  }
//  return ! err;
   frm.elements['city'].style="border:1px solid #1B100C";
   frm.elements['consignee'].style="border:1px solid #1B100C";
   frm.elements['address'].style="border:1px solid #1B100C";
   frm.elements['mobile'].style="border:1px solid #1B100C";
   frm.elements['bdate'].style="border:1px solid #1B100C";
   
   if (frm.elements['consignee'] && frm.elements['consignee'].value == 0)
   {
       frm.elements['consignee'].style="border:1px solid red";
	   return false;
   }
   if (frm.elements['city'] && frm.elements['city'].value == 0)
   {
       frm.elements['city'].style="border:1px solid red";
	   return false;
   }
    if (frm.elements['address'] && Utils.isEmpty(frm.elements['address'].value))
  {
  ...
  if (Utils.isEmpty(frm.elements['mobile']) || frm.elements['mobile'].value.length < 8 || (!Utils.isTel(frm.elements['mobile'].value))){
       frm.elements['mobile'].style="border:1px solid red";
	   return false;
   }
    if (frm.elements['bdate'] && Utils.isEmpty(frm.elements['bdate'].value))
  {
       frm.elements['bdate'].style="border:1px solid red";
	   return false;
   }
   return true;
}
