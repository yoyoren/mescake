...
function edit_profile($profile)
{
    ...
    /* 过滤非法的键值 */
    $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone','homeaddress','province');
...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, office_phone, home_phone, mobile_phone,  province, homeaddress FROM " .$GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";
	...
	$info['homeaddress'] = $infos['homeaddress'];
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, shipping_time, pay_status, add_time,best_time, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee, best_time ".
...
//        }
		...
		$row['goods'] = $GLOBALS['db']->getAll('select goods_id,goods_name,goods_sn, goods_price from ' . $GLOBALS['ecs']->table('order_goods') . " where order_id='$row[order_id]'" );

        $row['str_shipping_time'] = $row['shipping_time'] >0 ?local_date($GLOBALS['_CFG']['time_format'], $row['shipping_time']) : 'N';

        $arr[] = array('order_id'       => $row['order_id'],
                       'order_sn'       => $row['order_sn'],
			           'goods'       => $row['goods'],
			           'str_shipping_time'       => $row['str_shipping_time'],
